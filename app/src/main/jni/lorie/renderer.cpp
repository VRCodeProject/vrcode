...
static GLuint loadShader(GLenum shaderType, const char* pSource) {
    ...
    if (shader) {
        glShaderSource(shader, 1, &pSource, nullptr); checkGlError("glShaderSource");
        ...
        if (!compiled) {
            ...
            if (infoLen) {
                ...
                if (buf) {
                    glGetShaderInfoLog(shader, infoLen, nullptr, buf); checkGlError("glGetShaderInfoLog");
    ...
    if (program) {
        ...
        if (linkStatus != GL_TRUE) {
            ...
            if (bufLength) {
                ...
                if (buf) {
                    glGetProgramInfoLog(program, bufLength, nullptr, buf); checkGlError("glGetProgramInfoLog");
...
void LorieRenderer::set_cursor(LorieSurface *surface, uint32_t part_hotspot_x, uint32_t part_hotspot_y) {
	LOGV("Setting surface %p as cursor", surface);
	if (cursor_surface) cursor_surface->texture.uninit();
	cursor_surface = surface;
	this->hotspot_x = part_hotspot_x;
	this->hotspot_y = part_hotspot_y;

	requestRedraw();
}

void LorieRenderer::idleDraw(void *data) {
	auto* r = static_cast<LorieRenderer*> (data);
	
	r->redraw();
}

void LorieRenderer::drawCursor() {
	if (toplevel_surface == nullptr || cursor_surface == nullptr) return;
...
void LorieTexture::reinit() {
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr); checkGlError("glTexImage2D");
}
