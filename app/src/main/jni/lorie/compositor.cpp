...
void LorieCompositor::post(const std::function<void()>& f) {
    queue.write(f);
}

struct wl_event_source* LorieCompositor::add_fd_listener(int fd, uint32_t mask, wl_event_loop_fd_func_t func, void *data) const {
...
void LorieCompositor::terminate() const {
